'use client';

import React from 'react';
import Link from 'next/link';
import { Search } from 'lucide-react';
import { useNavbarSearch } from '@/hooks/use-navbar-search';
import { useBanner } from '@/hooks/use-banner';
import { NavLinks } from './navbar/NavLinks';
import { SearchResults } from './navbar/SearchResults';
import { AppBanner } from './navbar/AppBanner';

export function Navbar() {
    const {
        search,
        setSearch,
        results,
        isLoading,
        showResults,
        dropdownRef,
        handleSearchSubmit,
        handleResultClick,
    } = useNavbarSearch();

    const { isVisible: isBannerVisible, isAndroid } = useBanner();

    const appWebOfficial = process.env.NEXT_PUBLIC_APP_WEB_OFFICIAL || 'https://www.kawaiianimes.app/';
    const appApkUrl = process.env.NEXT_PUBLIC_APP_APK_URL || 'https://github.com/animelhd/kawaii-animes/raw/main/app-release.apk';

    return (
        <nav className="fixed top-0 z-50 w-full border-b border-border bg-background/80 backdrop-blur-xl">
            <div className="mx-auto flex h-14 max-w-[1600px] items-center justify-between px-6 lg:px-10">
                <div className="flex items-center gap-10">
                    <Link href="/" className="flex items-center">
                        <span className="text-xl font-black tracking-tighter text-foreground">
                            ANIME<span className="text-primary">LHD</span>
                        </span>
                    </Link>

                    <NavLinks />
                </div>

                <div className="flex items-center gap-4">
                    <div className="relative" ref={dropdownRef}>
                        <form onSubmit={handleSearchSubmit}>
                            <Search className="absolute top-1/2 left-3 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
                            <input
                                type="text"
                                value={search}
                                onChange={(e) => setSearch(e.target.value)}
                                placeholder="Buscar anime..."
                                className="h-9 w-40 rounded-full bg-white/5 pr-4 pl-9 text-[11px] font-bold text-white ring-1 ring-white/10 transition-all outline-none focus:w-60 focus:bg-white/10 focus:ring-primary/50 lg:w-64"
                            />
                        </form>

                        {showResults && (
                            <SearchResults 
                                isLoading={isLoading}
                                results={results}
                                search={search}
                                onResultClick={handleResultClick}
                            />
                        )}
                    </div>
                </div>
            </div>

            <AppBanner 
                isVisible={isBannerVisible}
                isAndroid={isAndroid}
                appWebOfficial={appWebOfficial}
                appApkUrl={appApkUrl}
            />
        </nav>
    );
}
